...
/***** ---- imageIndexCtrl functions ----- *****/
/* Provide the imageType for drop downs */
myApp.controller("imageSelectMediaCtrl",function($scope, ImageUrl, crossvars) {

    var imageSelectMedia = this;
    console.log("crossvars=" +  crossvars)
    imageSelectMedia.uploaded_media = crossvars.uploaded_media;
    console.log("crossvars.uploaded_media=" +  crossvars.uploaded_media);
    console.log("imageSelectMedia.uploaded_media = " +  imageSelectMedia.uploaded_media);

    /*         **** SETUP THE CONSTANTS ***   */
    /*   Setup for each image what its tile color, col and row span should be  */
    ...
    ImageUrl.query(function (data) {
        ...
        for (var i = 0; i < $scope.images.length; i++) {
            span = 2;
            $scope.images[i].color = COLORS[Math.floor(Math.random() * COLORS.length)];
    ...
    $scope.board_link = function (image) {
    ...
    $scope.createImage = function (image) {
        ImageUrl.save(image)
            ...
            .then(function (image) {
                ...
                $scope.image = {};
                console.log("set image.name=" + image.name);
                crossvars.uploaded_media = image.name;
                console.log("set crossvars.uploaded_media=" + crossvars.uploaded_media);
                $state.go('select_media');
            },
            function() {
